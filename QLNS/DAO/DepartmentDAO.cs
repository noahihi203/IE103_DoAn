...
namespace QLNS.DAO
{
    internal class DepartmentDAO
    {
        ...
        {
            string queryLayTenPhongBan = string.Format("SELECT * FROM PHONGBAN WHERE MAPB = {0}", id);
            DataTable data = DataProvider.Instance.executeQuery(queryLayTenPhongBan);
            DataRow dr = data.Rows[0];
            return (dr["TENPB"]).ToString();
        }
